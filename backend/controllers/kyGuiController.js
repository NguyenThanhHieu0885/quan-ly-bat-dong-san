...
const getKyGuiById = async (req, res) => {
  try {
    const { id } = req.params;
    const kyGui = await KyGui.findById(id);
    if (kyGui) {
...
const updateKyGui = async (req, res) => {
  try {
    ...
    const existingKyGui = await KyGui.findById(id);
    if (!existingKyGui) {
    ...
    await KyGui.update(id, req.body);
...
const deleteKyGui = async (req, res) => {
  try {
    ...
    const kyGuiToDelete = await KyGui.findById(id);
    if (!kyGuiToDelete) {
